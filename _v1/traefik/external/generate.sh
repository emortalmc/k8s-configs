...
# References to audiences are to Cloudflare Access audiences
...
if [ "$STAGING" == "true" ]; then
  ...
  audiences=("${audiences_staging[@]}")
elif [ "$STAGING" == "false" ]; then
  host="emortal.dev"
  routes=("${routes_prod[@]}")
  audiences=("${audiences_prod[@]}")
else
...
do
  ...
  sed "s|{{host}}|$host|g" "./$route.yaml" | sed "s|{{audience}}|$audience|g" | kubectl apply -f -
done
